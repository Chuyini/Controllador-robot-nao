...
static WbDeviceTag rig;//sensor creado mano de la derecha
...
static void find_and_enable_devices() {
  ...
  // ultrasound sensors
  rig=wb_robot_get_device("ds_right");
  us[0] = wb_robot_get_device("Sonar/Left");
  us[1] = wb_robot_get_device("Sonar/Right");
  int i;
  wb_distance_sensor_enable(rig, time_step);
  for (i = 0; i < 2; i++)
...
static void print_ultrasound_sensors() {
  ...
  double rr;
  int i;
  rr=wb_distance_sensor_get_value(rig);
  for (i = 0; i < 2; i++)
    dist[i] = wb_distance_sensor_get_value(us[i]);

  printf("-----ultrasound sensors-----\n");
  printf("left: %f m, right %f m, rr %f m\n", dist[0], dist[1],rr);
}
...
int main() {
 ...
 /// print_help();
  ...
  // walk forwards
  // wbu_motion_play(hand_wave);//esta funcion la use porque tenia el brazo arriba y fue mas facil usar esta funcion para  que la bajara
  double fsr1_value = wb_distance_sensor_get_value(us[1]);
  double derecha=wb_distance_sensor_get_value(rig);
  while (wb_robot_step(time_step) != -1) {
  // Obtener el valor del sensor fsr[1]
  fsr1_value = wb_distance_sensor_get_value(us[1]);
  derecha=wb_distance_sensor_get_value(rig);
 print_ultrasound_sensors();
  // Si el valor es mayor que un umbral, detener el robot
  fsr1_value = wb_distance_sensor_get_value(us[1]);
  
       if(1 )//esta funcion hace que camine siempre
       {
       wbu_motion_is_over(turn_right_60);
       wbu_motion_is_over(turn_left_60);
         wbu_motion_set_loop(turn_left_60, false);
        wbu_motion_set_loop(turn_right_60, false);
        
        wbu_motion_set_loop(forwards, true);
        wbu_motion_play(forwards);
       
       //derecha<0.4 quiere decir que no hay nada a la derecha
        //fsr1_value>0.3 quiere decir que si  hay algo
       
       } if(derecha<0.4 && fsr1_value>0.3 ){//volterará a la derecha si no detecta nunguna pared
   
       //derecha
       wbu_motion_is_over(forwards);
       wbu_motion_is_over(turn_left_60);
        wbu_motion_set_loop(forwards, false);
        wbu_motion_stop(forwards);
        wbu_motion_set_loop(turn_left_60, false);
        wbu_motion_stop(turn_left_60);
        wbu_motion_set_loop(turn_right_60, true);
        wbu_motion_play(turn_right_60);
       
  
  
  
  }if(fsr1_value<0.28 &&derecha>0.4){//volteara a la izquierda si exiten paredes intercediendo en el camino del robot
       ...
       wbu_motion_is_over(turn_right_60);
        ...
        wbu_motion_stop(turn_right_60);
        wbu_motion_set_loop(turn_left_60, true);
        wbu_motion_play(turn_left_60);
